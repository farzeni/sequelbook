...
// NewSQLiteExecutor creates a new SQLite query executor.
func NewSQLiteExecutor(resolver ConnectionResolver) *SQLiteExecutor {
	return &SQLiteExecutor{connResolver: resolver}
}

// Execute runs a SQL query against the specified connection.
func (e *SQLiteExecutor) Execute(ctx context.Context, connID string, sql string) (*Result, error) {
	if e.closed.Load() {
	...
	if dbType != connection.DBTypeSQLite {
		return nil, NewConnectionError(connID, fmt.Errorf("SQLiteExecutor requires SQLite connection, got %s", dbType))
	}
	...
	columns, err := columnsFromSQLRows(rows, sqliteTypeToDisplayType)
	if err != nil {
		return nil, e.handleExecutionError(queryCtx, queryID, sql, err)
	}
...
func (e *SQLiteExecutor) Cancel(queryID string) error {
...
func (e *SQLiteExecutor) handleExecutionError(ctx context.Context, queryID, sql string, err error) error {
